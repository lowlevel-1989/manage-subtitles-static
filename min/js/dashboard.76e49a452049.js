...
$("#cookieModal").modal("hide")}}})});var vhHeight=$("body").height();var fixHeight=vhHeight-window.innerHeight;$("body").css({overflow:'hidden'});if(vhHeight>0){$(".fix-container").css({height:(vhHeight-fixHeight)});};$(document).ready(()=>{var metadata=$("#metadata")
...
$(document).on("click",(event)=>{if(!$(event.target).is(".metadata")){metadata.removeClass("d-block d-lg-none animate__animated animate__slideInRight")}})});$(document).ready(()=>{let imdb_storage=new RLocalStorage(sm_conf["IMDB_LIST_STORAGE_PREFIX"],sm_conf["IMDB_LIST_STORAGE_NAME"],sm_conf["IMDB_LIST_STORAGE_LIMIT"])
...
$("#s_form").attr("action",qs_param);$("#s_form").submit()},});let _cookie=new MCookie('collapse')
$('.collapse').on('show.bs.collapse',function(){_cookie.set("subtitle","show",0)});$('.collapse').on('hide.bs.collapse',function(){_cookie.set("subtitle","hide",0)});});$("#form-add-subtitle").on("submit",(event)=>{event.preventDefault();let qs_param=sprintf("/dashboard/subtitle/{imdb}/?movie={movie}&season={season}",{"movie":encodeURIComponent($("#id_movie").val()),"imdb":$("#id_imdb").val(),"season":$("#id_season").val(),})
$(event.target).attr("action",qs_param);$(event.target).unbind('submit').submit();});;