...
let qs_param=sprintf("/dashboard/subtitle/{imdb}/?movie={movie}",{"movie":encodeURIComponent(ui.item.value),"imdb":ui.item.imdb,}).replaceAll('//','/')
$("#s_form").attr("action",qs_param);$("#s_form").submit()},});$("#form-add-subtitle").on("submit",(event)=>{event.preventDefault();url=new URL(window.location.href)
page=url.searchParams.get("page")
if(page<1){page=1}
let qs_param=sprintf("/dashboard/subtitle/{imdb}/?movie={movie}&season={season}&page={page}",{"movie":encodeURIComponent($("#id_movie").val()),"imdb":$("#id_imdb").val(),"season":$("#id_season").val(),"page":page,}).replaceAll('//','/')
