...
break}}
navegation_update()})});$(document).ready(()=>{var metadata=$("#metadata")
...
$(document).on("click",(event)=>{if(!$(event.target).is(".metadata")){metadata.removeClass("d-block d-lg-none animate__animated animate__slideInRight")}})});var vhHeight=$("body").height();var fixHeight=vhHeight-window.innerHeight;$("body").css({overflow:'hidden'});if(vhHeight>0){$(".fix-container").css({height:(vhHeight-fixHeight)});};$(document).ready(()=>{let _cookie=new MCookie("privacy")
$("#CookiesOk").focus()
...
$("#cookieModal").modal("hide")}}})});$(document).ready(()=>{let imdb_storage=new RLocalStorage(sm_conf["IMDB_LIST_STORAGE_PREFIX"],sm_conf["IMDB_LIST_STORAGE_NAME"],sm_conf["IMDB_LIST_STORAGE_LIMIT"])
